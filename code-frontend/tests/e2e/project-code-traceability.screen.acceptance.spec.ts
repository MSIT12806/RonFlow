...
test.describe('RonFlow UI/UX 驗收規格 - Project Code Traceability Query', () => {
  test('使用者可以從 Project Board 查詢跨 Task 的程式修改紀錄', async ({ page }, testInfo) => {
    ...
    await page.getByTestId('code-traceability-result').click()
    await expect(detailDialog).toBeVisible()
    await expect(detailDialog.getByRole('button', { name: '編輯', exact: true })).not.toBeVisible()
    await expect(detailDialog.getByRole('button', { name: '更多操作', exact: true })).not.toBeVisible()
    await expect(getTraceabilityCategory(detailDialog, 'api')).toContainText('GET /api/projects/{projectId}/code-traceability')
    await detailDialog.getByRole('button', { name: '關閉視窗', exact: true }).click()
