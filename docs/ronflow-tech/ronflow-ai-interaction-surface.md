...
1. `AiAuditRegistry` 已從 runtime-level registry 推進到 outbox + background consumer + audit read model；目前可支援跨 session 的查詢。若未來需要更完整治理，可再補 retention policy 與查詢權限策略。
