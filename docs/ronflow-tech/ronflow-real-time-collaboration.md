...
- [UpdateTaskCommandService.cs](../../code-backend/RonFlow.Application/UpdateTaskCommandService.cs)：修改 task 前先檢查是否持有 content edit lock
- [MoveTaskToTrashCommandService.cs](../../code-backend/RonFlow.Application/MoveTaskToTrashCommandService.cs)：lifecycle command 前先檢查 lock 衝突

domain layer 的核心 task / project 規則：
- [Task.cs](../../code-backend/RonFlow.Domain/Task.cs)：lifecycle state、reminders、activity timeline、state transition、detail update
...
- [UpdateTaskCommandService.cs](../../code-backend/RonFlow.Application/UpdateTaskCommandService.cs) 會在更新 task 前要求 current user 真的持有 lock
