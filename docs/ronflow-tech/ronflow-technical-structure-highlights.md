...
對應的實踐細節文章整理在 [RonFlow Tech](./README.md)。
...
- [x] reporting / projection（第一批）
    ...
    - 實踐細節見 [RonFlow 的 reporting / projection 實踐](./ronflow-reporting-projection.md)
- [x] SQLite-to-Git persistence sync
    - 單一使用者、多裝置的 personal sync 模式
    - runtime SQLite 與獨立 Git repository 之間的 snapshot / commit / push 流程
    - spec: [RonFlow Database Git Sync Spec](../specs/ronflow-database-git-sync-spec.md)
- [x] Task mutation lock policy 與 aggregate execution result
    ...
    - 實踐細節見 [RonFlow 的多人協作 / 即時同步設計](./ronflow-real-time-collaboration.md)
- [x] AI interaction surface（RonFlow v0.3 baseline）
    - spec: [RonFlow AI Interaction Surface Spec](../specs/ronflow-ai-interaction-surface-spec.md)
    ...
    - 實踐細節見 [RonFlow 的 AI discovery surface](./ronflow-ai-discovery-surface.md) 與 [RonFlow 的 AI interaction surface](./ronflow-ai-interaction-surface.md)
...
- [x] AI audit trail 持久化與查詢化
    ...
    - 實踐細節見 [RonFlow 的 AI audit trail 持久化與查詢化](./ronflow-ai-audit-projection-read-model.md)
