...
```text
1. RonFlow 啟動時先 pull database repository
...
```text
1. RonFlow 啟動
2. 先同步 database repository 的最新內容
3. 如果 repo 內已有 ronflow.db，將其複製到 runtime DB 位置
4. runtime DB 建立完成後，再初始化 RonFlow API 與 SQLite tables
5. 若同步失敗，應保留清楚的 error log，但不要破壞本地可啟動性
```
...
2. 產生一致性 snapshot
3. 將 snapshot 寫回 database repository 的 ronflow.db
4. git add
5. git commit
6. git push
7. 若 push 失敗，保留 pending/error 狀態，讓下一次啟動或下一次異動重試
```
...
7. 啟動時的 pull 是預設行為。
8. 每次資料異動後的 push 是預設行為。
9. 若 Git sync 失敗，不得把本地 mutation 視為失敗。
10. 若 repository 發生外部衝突，第一版採手動解決，不自動 merge。
...
3. File system copy / atomic replace
4. 未來可能的 MQ dispatch
5. 未來可能的 polling / background worker
```
...
4. implement layer 可以在 mutation 後呼叫 sync port
```
...
```text
1. 啟動 RonFlow 後，runtime DB 會先與 database repo 對齊
