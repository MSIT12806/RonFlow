...
12. 若 Task Detail summary 的 `workflow_state_key` 是 `Todo`，必須包含 `recommended_start_work_apply:` 區塊，直接提供 `move_task_state` 到 `Active` 的 apply 欄位，並說明只在即將實作 task 時使用。`r`n13. Task Detail summary 必須包含 `code_traceability_summary:`，且至少列出 api、frontendPages、frontendComponents 的 `item_count` 與 target 摘要。`r`n14. JSON summary 必須包含 `version`、`canonicalEndpoint` 與可直接消費的 id / count / nextActions 欄位。
```
